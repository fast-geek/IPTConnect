...
def distribute_bonus_points(points_list):
    ...
    if max_points == equal_count:
        return [1.0 for _ in range(equal_count)]
    head = [(equal_count + 1.0) / equal_count for _ in range(equal_count)]
    equal_count = points_list.count(points_list[max_points-1])
    tail = [(equal_count - 1.0) / equal_count for _ in range(equal_count)]
    return head + [1.0 for _ in range(max_points - (len(head)+len(tail)))] + tail
