...
jurygrades = JuryGrade.objects.filter(round__pf_number=1) | JuryGrade.objects.filter(
    round__pf_number=2) | JuryGrade.objects.filter(round__pf_number=3) | JuryGrade.objects.filter(
    round__pf_number=4)  # discard the final
...
for indt, team in enumerate(teams):
    team.jurys = []
    mygrades = jurygrades.filter(round__reporter_team__name=team.name) | jurygrades.filter(
        round__opponent_team__name=team.name) | jurygrades.filter(round__reviewer_team__name=team.name)
    for indj, jury in enumerate(jurys):
        ...
        for jgrade in jgrades:
            if jgrade.round.reporter_team.name == team.name:
                repgrades = [jg.grade_reporter for jg in JuryGrade.objects.filter(round__reporter_team__name=team.name)
            ...
            if jgrade.round.opponent_team.name == team.name:
                oppgrades = [jg.grade_opponent for jg in JuryGrade.objects.filter(round__opponent_team__name=team.name)
                             if jg.round == jgrade.round]
            ...
            if jgrade.round.reviewer_team.name == team.name:
                revgrades = [jg.grade_reviewer for jg in JuryGrade.objects.filter(round__reviewer_team__name=team.name)
                             if jg.round == jgrade.round]
        ...
        maxscatter = 1.0
        if len(grads) > 0:
            ax.scatter(indt, indj, s=np.sum([v[2] for v in vals] * 15), c=color,
                       alpha=max(min(np.abs(np.mean(gradscatters)) / maxscatter, 1), 0.4), linewidths=0.2)
